...
async function processJsonFile(filePath) {
  ...
  try {
    ...
    if (data.lessons && Array.isArray(data.lessons)) {
      for (const lesson of data.lessons) {
        if ((lesson.type === 'expression' || lesson.type === 'single_expression') && lesson.expression) {
          ...
          // 베트남어 필드 삭제
          if (expr.meaningVi) {
            delete expr.meaningVi;
            removalCount++;
          }
          if (expr.explanationVi) {
            delete expr.explanationVi;
            removalCount++;
          }
          if (expr.whenToUseVi) {
            delete expr.whenToUseVi;
            removalCount++;
          }
          if (expr.culturalTipVi) {
            delete expr.culturalTipVi;
            removalCount++;
          }

          // examples 배열의 베트남어 삭제
          if (expr.examples && Array.isArray(expr.examples)) {
            for (const example of expr.examples) {
              if (example.meaningVi) {
                delete example.meaningVi;
                removalCount++;
              }
            }
          }
          ...
          // explanation 영어 번역
          if (expr.explanation && !expr.explanationEn) {
          ...
          // whenToUse 영어 번역
          if (expr.whenToUse && !expr.whenToUseEn) {
          ...
          // culturalTip 영어 번역
          if (expr.culturalTip && !expr.culturalTipEn) {
    ...
    console.log(`  ✅ Removed ${removalCount} Vietnamese, Added ${translationCount} English`);

    return { translationCount, removalCount };
  } catch (error) {
    console.error(`  ❌ Error: ${error.message}`);
    return { translationCount: 0, removalCount: 0 };
  }
}

// 메인 함수
async function main() {
  console.log('🚀 Starting translation for ALL folders (client + server)...\n');
  ...
  console.log(`   English translations added: ${totalTranslations}`);
}
